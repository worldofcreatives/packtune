...
// Fetch all opportunities for a specific user

  export const fetchUserOpportunities = () => async (dispatch) => {
  dispatch(fetchUserOpportunitiesRequest());
  try {
    const response = await fetch('/api/opportunities/browse');
...
const initialState = {
  ...
  const userOppsReducer = (state = initialState, action) => {
    switch (action.type) {
  ...
  export default userOppsReducer;
